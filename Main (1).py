...
confusion_matrix = metrics.confusion_matrix(cm_xgb, cm_test)
cmp = metrics.ConfusionMatrixDisplay(confusion_matrix=confusion_matrix, 
display_labels=['DEF', 'OK'])
cmp.plot()
plt.title("XGB")
...
if (score_svc > score_xgb):
  ...
  Cvalue = dict['C']
  window.destroy()
  Accuracies.show(score_svc, score_xgb, score_gnb, textb, 
precision_svc, precision_xgb, precision_gnb, recall_svc, recall_xgb, 
recall_gnb, f1_svc, f1_xgb, f1_gnb, data, labels, Gvalue, Cvalue)
main_page()
def calc():
  from tkinter import filedialog
  import numpy as np
